...
import { CostCenterDTO } from './cost-center.dto.js';
...
export class CostCenterEntity {
  ...
  assertIdentity(): void {
    ...
    if (this.annualBudget < 0 || !Number.isFinite(this.annualBudget)) {
      throw new SmsDomainError('CostCenter.annualBudget invalid');
    }
    if (this.budgetThresholdAlert < 0 || this.budgetThresholdAlert > 1) {
      throw new SmsDomainError('CostCenter.budgetThresholdAlert invalid');
    }
    if (this.budgetSensitivityIndex < 0 || this.budgetSensitivityIndex > 1) {
      throw new SmsDomainError('CostCenter.budgetSensitivityIndex invalid');
    }
    if (this.percentage < 0 || this.percentage > 100) {
      throw new SmsDomainError('CostCenter.percentage invalid');
    }
    if (this.renewableEnergyTarget < 0 || this.renewableEnergyTarget > 100) {
      throw new SmsDomainError('CostCenter.renewableEnergyTarget invalid');
    }
  }
