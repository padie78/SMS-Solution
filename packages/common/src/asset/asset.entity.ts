...
export class AssetEntity {
  ...
  assertHierarchy(): void {
    ...
    if (!this.installationDate?.trim()) throw new SmsDomainError('Asset.installationDate required');
    if (!Number.isFinite(this.nominalPowerKw) || this.nominalPowerKw < 0) {
      throw new SmsDomainError('Asset.nominalPowerKw invalid');
    }
    if (this.powerFactorTarget < 0 || this.powerFactorTarget > 1) {
      throw new SmsDomainError('Asset.powerFactorTarget invalid');
    }
    if (this.biogenicFraction < 0 || this.biogenicFraction > 1) {
      throw new SmsDomainError('Asset.biogenicFraction invalid');
    }
    if (this.dataQualityScore < 0 || this.dataQualityScore > 1) {
      throw new SmsDomainError('Asset.dataQualityScore invalid');
    }
    if (!Number.isFinite(this.nominalEfficiency) || this.nominalEfficiency < 0) {
      throw new SmsDomainError('Asset.nominalEfficiency invalid');
    }
    const dcy = this.dutyCycleExpected;
    if (!(dcy >= 0 && dcy <= 1)) throw new SmsDomainError('Asset.dutyCycleExpected invalid');
  }
