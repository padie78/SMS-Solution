...
import { EmissionFactorDTO } from './emission-factor.dto.js';
...
export class EmissionFactorEntity {
  ...
  assertIdentity(): void {
    if (!this.id?.trim()) throw new SmsDomainError('EmissionFactor.id required');
    ...
    if (!this.source?.trim()) throw new SmsDomainError('EmissionFactor.source required');
    if (!Number.isFinite(this.year)) throw new SmsDomainError('EmissionFactor.year invalid');
    if (this.value < 0 || !Number.isFinite(this.value)) {
      throw new SmsDomainError('EmissionFactor.value invalid');
    }
    if (
      ...
      throw new SmsDomainError('EmissionFactor.uncertaintyPercentage invalid');
    }
    if (!Number.isFinite(this.decarbonizationTrend)) {
      throw new SmsDomainError('EmissionFactor.decarbonizationTrend invalid');
    }
  }
