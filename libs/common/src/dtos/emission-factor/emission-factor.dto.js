...
function coerceActivityType(input) {
    const s = String(input ?? '')
        .trim()
        .toUpperCase();
    if (!s)
        return input;
    if (s === 'ELEC' || s === 'ELECTRICITY' || s === 'ELECTRIC')
        return 'ELECTRICITY';
    if (s === 'GAS' || s === 'NATURAL_GAS' || s === 'NATURAL-GAS')
        return 'NATURAL_GAS';
    if (s === 'DIESEL' || s === 'FUEL')
        return 'DIESEL';
    if (s === 'FLIGHT')
        return 'FLIGHT';
    if (s === 'REFRIGERANT' || s === 'F_GAS')
        return 'REFRIGERANT';
    if (s === 'WASTE')
        return 'WASTE';
    if (s === 'LOGISTICS' || s === 'TRANSPORT')
        return 'LOGISTICS';
    return input;
}
function coercePhysicalUnit(input) {
    const raw = String(input ?? '').trim();
    if (!raw)
        return input;
    const u = raw.toUpperCase().replace(/\s+/g, '_');
    if (raw.toLowerCase() === 'kg/kwh' || u === 'KG/KWH')
        return 'KWH';
    if (u === 'KWH' || u === 'KW·H')
        return 'KWH';
    return u.replace(/-/g, '_');
}
function coerceCalculationMethod(input) {
    const s = String(input ?? '')
        .trim()
        .toUpperCase()
        .replace(/-/g, '_');
...
export class EmissionFactorDTO {
    ...
    constructor(id, name, year, regionCode, activityType, scope, calculationMethod, unit, value, co2Value, ch4Value, n2oValue, hfcValue, gwpReference, source, sourceUrl, dataQualityTier, uncertaintyPercentage, externalAuditDate, isProjection, decarbonizationTrend, biologicalCarbonStorage, status, tags, createdAt, updatedAt) {
        ...
        if (ch4Value !== null && ch4Value !== undefined)
            this.ch4Value = ch4Value;
        if (n2oValue !== null && n2oValue !== undefined)
            this.n2oValue = n2oValue;
        if (hfcValue !== null && hfcValue !== undefined)
        ...
        if (externalAuditDate?.trim())
            this.externalAuditDate = externalAuditDate.trim();
        this.isProjection = isProjection ?? false;
        this.decarbonizationTrend = decarbonizationTrend ?? 0;
        if (biologicalCarbonStorage !== null &&
...
export const parseEmissionFactorDTO = (input) => {
    const d = EmissionFactorDTOSchema.parse(input);
    const id = d.id?.trim() ? d.id.trim() : generateEmissionFactorId();
    return new EmissionFactorDTO(id, d.name, d.year, d.regionCode, d.activityType, d.scope, d.calculationMethod, d.unit, d.value, d.co2Value, d.ch4Value, d.n2oValue, d.hfcValue, d.gwpReference, d.source, d.sourceUrl, d.dataQualityTier, d.uncertaintyPercentage, d.externalAuditDate, d.isProjection, d.decarbonizationTrend, d.biologicalCarbonStorage, d.status, d.tags, d.createdAt, d.updatedAt);
};
export const safeParseEmissionFactorDTO = (input) => EmissionFactorDTOSchema.safeParse(input);
//# sourceMappingURL=emission-factor.dto.js.map