...
export type InvoicePersistence = {
  ...
  tex: InvoiceDTO['technicalExtraction'];
  pat: string;
  tdp: number;
};
...
export const InvoiceMapper = Object.freeze({
  ...
  persistenceToDTO(row: InvoicePersistence): InvoiceDTO {
    return {
      ...
      metadata: row.mt,
      technicalExtraction: row.tex ?? InvoiceIaTechnicalExtractionSchema.parse({}),
      processedAt: row.pat,
