...
export class BuildingEntity {
  ...
  assertHierarchy(): void {
    if (!this.id?.trim()) throw new DomainInvariantError('Building.id required');
    if (!this.organizationId?.trim()) throw new DomainInvariantError('Building.organizationId required');
    if (!this.regionId?.trim()) throw new DomainInvariantError('Building.regionId required');
    if (!this.branchId?.trim()) throw new DomainInvariantError('Building.branchId required');
    if (!this.name?.trim()) throw new DomainInvariantError('Building.name required');
