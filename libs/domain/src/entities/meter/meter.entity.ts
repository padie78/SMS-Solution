...
export class MeterEntity {
  ...
  private validateHierarchy(): void {
    if (!this.id?.trim()) throw new DomainInvariantError('Meter.id required');
    if (!this.orgId?.trim()) throw new DomainInvariantError('Meter.orgId required');
    if (!this.regionId?.trim()) throw new DomainInvariantError('Meter.regionId required');
    if (!this.branchId?.trim()) throw new DomainInvariantError('Meter.branchId required');
    if (!this.buildingId?.trim()) {
      throw new DomainInvariantError('Meter cannot exist without buildingId (localization tree)');
    }
    if (!this.serialNumber?.trim()) throw new DomainInvariantError('Meter.serialNumber required');
    if (!this.name?.trim()) throw new DomainInvariantError('Meter.name required');
