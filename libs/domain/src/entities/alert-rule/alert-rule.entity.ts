...
export class AlertRuleEntity {
  ...
  assertIdentity(): void {
    if (!this.id?.trim()) throw new DomainInvariantError('AlertRule.id required');
    if (!this.orgId?.trim()) throw new DomainInvariantError('AlertRule.orgId required');
    if (!this.branchId?.trim()) throw new DomainInvariantError('AlertRule.branchId required');
    if (!this.entityId?.trim()) throw new DomainInvariantError('AlertRule.entityId required');
    if (!this.name?.trim()) throw new DomainInvariantError('AlertRule.name required');
    if (!this.metricName?.trim()) throw new DomainInvariantError('AlertRule.metricName required');
    if (!Number.isFinite(this.threshold)) throw new DomainInvariantError('AlertRule.threshold invalid');
    if (
