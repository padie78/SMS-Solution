...
export class ProductionLogEntity {
  ...
  assertIdentity(): void {
    if (!this.orgId?.trim()) throw new DomainInvariantError('ProductionLog.orgId required');
    if (!this.branchId?.trim()) throw new DomainInvariantError('ProductionLog.branchId required');
    if (!this.period?.trim()) throw new DomainInvariantError('ProductionLog.period required');
  }
