...
import { buildRadarChartOption } from '../../../features/dashboard/lib/global-dashboard-chart-options';
...
@Component({
  selector: 'app-global-dashboard-radar-chart',
  standalone: true,
  ...
  template: `
    ...
    >
      <div>
        <ui-dashboard-metric-label text="Health score" hint="Armónicos · CO₂e · RUL · Eficiencia turno" />
        <h3 class="text-sm font-black text-slate-900 uppercase tracking-wide mt-1 m-0">Capa estratégica</h3>
      </div>
...
export class GlobalDashboardRadarChartComponent {
  private readonly svc = inject(GlobalDashboardStateService);
  readonly options = computed(() => buildRadarChartOption(this.svc.state()));
}
